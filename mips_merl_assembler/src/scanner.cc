...
std::vector<Token> scan(const std::string &input) {
  ...
  for (auto &token : tokens) {
    if (token.getKind() == Token::WORD) {
      ...
      } else if (token.getLexeme() == ".import") {
        // Convert WORD token to IMPORT token
        newTokens.push_back(Token(Token::IMPORT, token.getLexeme()));
      } else if (token.getLexeme() == ".export") {
        // Convert WORD token to EXPORT token
        newTokens.push_back(Token(Token::EXPORT, token.getLexeme()));
      } else {
  ...
  // Post-process to convert ID tokens that follow IMPORT/EXPORT to LABEL tokens
  std::vector<Token> finalTokens;
  for (size_t i = 0; i < newTokens.size(); ++i) {
    if ((newTokens[i].getKind() == Token::IMPORT || newTokens[i].getKind() == Token::EXPORT) 
        && i + 1 < newTokens.size() 
        && newTokens[i + 1].getKind() == Token::ID) {
      // Convert the following ID token to LABEL token
      finalTokens.push_back(newTokens[i]);
      finalTokens.push_back(Token(Token::LABEL, newTokens[i + 1].getLexeme()));
